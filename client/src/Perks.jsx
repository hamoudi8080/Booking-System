export default function Perks({selected, onChange}) {
    ...
    function handleCbClick(ev) {
       /* if (selected.includes(ev)) {
            onChange(selected.filter(p => p !== ev));
        } else {
            onChange([...selected, perk]);
        }*/
       const {checked, name} = ev.target;
       if (checked) {
        //If the checkbox is checked (checked is true), the perk is added to the selected array.
           onChange([...selected, name]);
       } else {
        //If the checkbox is unchecked (checked is false), the perk is removed from the selected array.
    ...
    return (
        <>
            ...
            {/* HTML checkboxes use the checked attribute to determine if they should be displayed with a checkmark.
            If selected.includes('parking') returns true, the checked attribute of the checkbox is set to true.
            */}
            ...
            <label className="border p-4 flex rounded-2xl gap-2 items-center cursor-pointer">
                <input type="checkbox" checked = {selected.includes('pentrance')} name="pentrance" onChange={handleCbClick} />
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
