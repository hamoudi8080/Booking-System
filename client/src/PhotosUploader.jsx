...
export default function PhotosUploader({ addedPhotos, onChange }) {
    ...
    function selectAsMainPhoto(ev, filename) {
        ev.preventDefault();
        //This expression creates a new array where the first element is filename.
        //The spread operator (...) then adds all the other photos (those that do not match filename) from the filtered array into this new array.
        //As a result, the new array starts with filename followed by the rest of the photos.
    ...
    return (
        <>
            ...
            <div className="mt-2 gap-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6">
                {addedPhotos.length > 0 && addedPhotos.map((link) => (
                    <div className=" xl:max-w-[240px] flex h-32  relative" key={link}>
                        <img className="rounded-2xl w-full  object-cover" src={'http://localhost:4000/uploads/' + link} alt="" />
                        <button onClick={(ev) => removePhoto(ev, link)} className=' absolute bottom-1 right-1 text-white bg-black py-2 px-3 cursor-pointer bg-opacity-50 rounded-2xl '>
                        ...
                        <button onClick={(ev) => selectAsMainPhoto(ev, link)} className=' absolute bottom-1 left-1 text-white bg-black py-2 px-3 cursor-pointer bg-opacity-50 rounded-2xl '>
